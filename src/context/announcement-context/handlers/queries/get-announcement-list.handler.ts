...
export class GetAnnouncementListQuery {
  constructor(
    ...
    public readonly hasSurvey?: boolean,
  ) {}
}
...
export class GetAnnouncementListHandler implements IQueryHandler<GetAnnouncementListQuery> {
  ...
  async execute(
    ...
    this.logger.debug(
      `공지사항 목록 조회 - 공개: ${isPublic}, 고정: ${isFixed}, 카테고리: ${categoryId}, 정렬: ${orderBy}, 페이지: ${page}, 제한: ${limit}, 마감제외: ${excludeExpired}, 검색: ${search}, 설문존재: ${hasSurvey}`,
    );
    ...
    if (hasSurvey !== undefined) {
      if (hasSurvey) {
        ...
        // 설문조사가 없는 공지사항만 필터링
        if (hasWhere) {
          queryBuilder.andWhere('survey.id IS NULL');
        } else {
          queryBuilder.where('survey.id IS NULL');
          hasWhere = true;
        }
      }
    }
    ...
    if (orderBy === 'order') {
      ...
      queryBuilder.addOrderBy('announcement.createdAt', 'DESC'); // order가 같으면 최신순
    } else {
