...
export class WikiFileSystemService {
   ...
   * @throws BadRequestException - 빈 경로인 경우
   */
  async 경로로_폴더를_조회한다(path: string): Promise<WikiFileSystem> {
    ...
    if (!trimmedPath || trimmedPath === '') {
      throw new BadRequestException(
        '루트 경로(/)는 사용할 수 없습니다. 최상위 폴더들을 조회하려면 폴더 구조 조회 API를 사용하세요.'
      );
    }
    ...
    // 최상위 폴더부터 순차적으로 폴더 찾기
    let currentParentId: string | null = null;
    let currentFolder: WikiFileSystem | null = null;

    for (let i = 0; i < folderNames.length; i++) {
      const folderName = folderNames[i];
      this.logger.log(`폴더 검색 중: "${folderName}" (부모 ID: ${currentParentId || '최상위'})`);
