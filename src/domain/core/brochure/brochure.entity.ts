...
import type { BrochureDto } from './brochure.types';

/**
 * 브로슈어 엔티티
 *
 * 회사 홍보 브로슈어 정보를 관리합니다.
 */
@Entity('brochure')
export class Brochure extends BaseEntity<BrochureDto> {
  @Column({
    type: 'varchar',
    length: 500,
    comment: '제목',
  ...
  /**
   * Brochure 엔티티를 DTO로 변환한다
   */
  DTO로_변환한다(): BrochureDto {
    return {
      id: this.id,
   ...
   * 상태를 변경한다
   */
  상태를_변경한다(status: BrochureStatus): void {
    this.status = status;
  }
}
