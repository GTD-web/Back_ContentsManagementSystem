...
| **8-1. 권한 로그 모달 제어<br>(다시 보지 않기)** | `GET /admin/announcements/permission-logs/unread`<br>`PATCH /admin/announcements/permission-logs/dismiss` | • DismissedPermissionLog<br>• AnnouncementPermissionLog | • `logType` = 'announcement'<br>• `permissionLogId` (FK)<br>• `dismissedBy` (관리자 ID)<br>• `dismissedAt` (timestamp)<br>• UK: (logType, permissionLogId, dismissedBy) | 1. 미열람 로그 조회 (모달용)<br>2. 관리자가 "다시 보지 않기" 클릭<br>3. DismissedPermissionLog 생성<br>4. 해당 관리자는 모달에서 제외<br>5. 다른 관리자는 계속 표시<br>6. 관리 페이지에서는 모든 로그 조회 가능 |
