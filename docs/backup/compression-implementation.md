...
#### 1. **xz 압축 기능 추가**
- child_process로 `xz` 명령어 사용
- 압축률: 평균 **70-90%** (10MB → 1-3MB, xz는 일반적으로 더 높은 압축률)
- 파일 형식: `.sql.xz`
...
| 파일 | 변경 사항 |
|------|----------|
| `backup.service.ts` | - xz 압축 로직 추가<br>- 압축률 계산 및 로깅<br>- 압축 활성화/비활성화 지원 |
| `backup.types.ts` | - `BackupConfig.compress` 추가<br>- `BackupResult`에 압축 정보 추가 |
| `backup-retention.service.ts` | - `.sql.xz` 파일 처리 지원 |
...
| 일간 (10개) | 50 MB | 5-15 MB | 35-45 MB/파일 | 10일 | 350-450 MB |
...
```
✅ daily 백업 성공
   파일명: backup_daily_20260121_153045.sql.xz
   원본 크기: 52.4 MB
   압축 크기: 5.2 MB
   압축률: 90.1%
```
...
```bash
# 직접 복구 (권장)
xz -dc backup_daily_20260121_153045.sql.xz | \
  psql -h localhost -U postgres -d lumir_cms

# 또는 2단계 복구
xz -d backup_daily_20260121_153045.sql.xz
...
### Linux/Mac
- ✅ `xz` / `unxz` (대부분의 배포판 기본 설치)

### Windows
- ✅ **7-Zip** (https://www.7-zip.org/) - 권장
- ✅ **Git Bash** (git 설치 시 포함)
- ✅ **PowerShell 7+** (`7z x` 또는 xz 설치 시 `xz -d`)
...
```bash
# 특정 파일의 압축 정보
xz -l backup_daily_20260121_153045.sql.xz
```
...
- ✅ 기존 `.sql` 백업과 새 `.sql.xz` 백업 모두 복구 가능
