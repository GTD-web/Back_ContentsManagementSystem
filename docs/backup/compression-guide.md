# 백업 압축 가이드

## 개요

백업 시스템은 **xz 압축**을 사용하여 백업 파일의 크기를 70-90% 줄입니다. xz는 gzip보다 일반적으로 더 높은 압축률을 달성합니다. 이는 특히 제한된 스토리지 환경에서 매우 유용합니다.

---

## 압축의 장점

### 💰 비용 절감

**압축 효과 예시** (실제 데이터 기준):

| 원본 SQL 크기 | 압축 후 크기 | 절감률 | 디스크 절약 |
|--------------|-------------|--------|------------|
| 100 MB | 10-30 MB | 70-90% | 70-90 MB |
| 500 MB | 50-150 MB | 70-90% | 350-450 MB |
| 1 GB | 100-300 MB | 70-90% | 700-900 MB |
| 10 GB | 1-3 GB | 70-90% | 7-9 GB |

**EC2 스토리지 비용 계산**:
- EC2 EBS gp3: 약 $0.08/GB/월
- 10GB 백업 → 압축 시 1-3GB
- 월간 절감: $0.56-$0.72 (7-9GB × $0.08)
- 연간 절감: $6.72-$8.64

### ⚡ 성능 향상

1. **백업 속도 개선**
   - 디스크 I/O 감소
   - 네트워크 전송 시간 단축 (원격 백업 시)

2. **복구 속도**
   - 작은 파일 크기로 빠른 전송
   - CPU 압축 해제는 매우 빠름 (초 단위)

### 🔒 보안

- 압축된 파일은 추가 암호화 적용 가능
- 바이너리 형식으로 인한 기본적인 난독화

---

## 압축 설정

### 환경변수

`.env` 파일에서 압축을 제어합니다:

```bash
# 압축 활성화 (기본값, 권장)
BACKUP_COMPRESS=true

# 압축 비활성화 (디버깅용)
BACKUP_COMPRESS=false
```

### 설정 변경 후 재시작

```bash
# 애플리케이션 재시작
npm run start:dev

# 또는 프로덕션에서
pm2 restart lumir-cms-backend
```

---

## 압축 파일 다루기

### 압축 파일 확인

```bash
# 파일 크기 확인
ls -lh ./backups/database/daily/

# 압축률 확인
xz -l backup_daily_20260121_010000.sql.xz

# 출력 예시:
#   Strms  Blocks   Compressed Uncompressed  Ratio  Check   Filename
#       1       1    2,621,440   10,485,760  25.0%  CRC64   backup_daily_20260121_010000.sql.xz
```

### 압축 해제

#### Linux/Mac

```bash
# 압축 해제 (원본 파일 생성)
xz -d backup_daily_20260121_010000.sql.xz
# 결과: backup_daily_20260121_010000.sql

# 압축 파일 유지하면서 해제
xz -dk backup_daily_20260121_010000.sql.xz

# 표준 출력으로 압축 해제 (파일 생성 안 함)
xz -dc backup_daily_20260121_010000.sql.xz > backup.sql
```

#### Windows

**방법 1: 7-Zip 사용 (권장)**

```powershell
# 7-Zip 설치 (https://www.7-zip.org/)
# GUI: 파일 우클릭 > 7-Zip > Extract Here

# 명령줄:
7z x backup_daily_20260121_010000.sql.xz
```

**방법 2: Git Bash**

```bash
# Git Bash 터미널에서
xz -d backup_daily_20260121_010000.sql.xz
```

**방법 3: PowerShell 7+ (내장)**

```powershell
# PowerShell 7 이상 (xz가 PATH에 있는 경우)
xz -d backup_daily_20260121_010000.sql.xz
```

### 내용 미리보기

```bash
# 압축 파일의 처음 100줄 확인
xz -dc backup_daily_20260121_010000.sql.xz | head -n 100

# 특정 테이블 검색
xz -dc backup_daily_20260121_010000.sql.xz | grep "CREATE TABLE users"
```

---

## 복구 방법

### 직접 복구 (권장)

압축 해제와 복구를 동시에 수행:

```bash
# Linux/Mac
xz -dc backup_daily_20260121_010000.sql.xz | \
  PGPASSWORD="password" psql -h localhost -U postgres -d lumir_cms

# Windows (Git Bash)
xz -dc backup_daily_20260121_010000.sql.xz | \
  PGPASSWORD="password" psql -h localhost -U postgres -d lumir_cms
```

### 2단계 복구

압축 해제 후 복구:

```bash
# 1. 압축 해제
xz -d backup_daily_20260121_010000.sql.xz

# 2. 복구
psql -h localhost -U postgres -d lumir_cms -f backup_daily_20260121_010000.sql

# 3. 정리 (선택사항)
rm backup_daily_20260121_010000.sql
```

---

## 압축 비활성화 (디버깅용)

### 언제 비활성화하나요?

압축을 비활성화하는 경우:
- 백업 파일 내용을 직접 확인해야 할 때
- 개발/테스트 환경에서 디버깅 시
- 압축 해제 도구가 없는 환경

### 비활성화 방법

```bash
# .env 파일 수정
BACKUP_COMPRESS=false
```

### 주의사항

⚠️ **프로덕션 환경에서는 권장하지 않습니다**:
- 디스크 공간을 10배 더 사용
- 백업 파일 크기가 매우 커짐
- EC2 스토리지 비용 증가

---

## 용량 모니터링

### 백업 디렉토리 크기 확인

```bash
# 전체 백업 크기
du -sh ./backups/database

# 타입별 크기
du -sh ./backups/database/*

# 출력 예시:
# 256M    ./backups/database/daily
# 128M    ./backups/database/four_hourly
# 512M    ./backups/database/monthly
# 1.2G    ./backups/database/quarterly
# 64M     ./backups/database/weekly
# 2.5G    ./backups/database/yearly
```

### 자동 알림 설정

디스크 사용량이 80%를 초과하면 알림:

```bash
# crontab에 추가
*/30 * * * * /scripts/check-disk-space.sh

# /scripts/check-disk-space.sh 내용:
#!/bin/bash
USAGE=$(df -h /backups | awk 'NR==2 {print $5}' | sed 's/%//')
if [ $USAGE -gt 80 ]; then
  echo "⚠️ 백업 디스크 사용량: ${USAGE}%" | mail -s "백업 디스크 알림" admin@example.com
fi
```

---

## FAQ

### Q: 압축으로 인한 성능 저하는 없나요?

A: **거의 없습니다**. xz 압축은:
- CPU 사용량: 매우 낮음 (백업 중 5-10% 증가)
- 백업 시간: 오히려 감소 (I/O 병목 감소)
- 전체 시스템 성능에 영향 없음

### Q: 압축률은 어떻게 되나요?

A: SQL 파일은 텍스트 형식이라 **평균 70-90%** 압축됩니다:
- 반복되는 SQL 구문 (INSERT INTO...)
- 긴 공백과 들여쓰기
- 유사한 데이터 패턴

### Q: 압축 해제에 오래 걸리나요?

A: **매우 빠릅니다**:
- 1GB 압축 파일: 약 5-10초
- 10GB 압축 파일: 약 30-60초
- 디스크 I/O가 주요 병목

### Q: 압축된 파일을 직접 확인할 수 있나요?

A: 네, `xz -dc`로 가능합니다:
```bash
# 처음 100줄 확인
xz -dc backup.sql.xz | head -n 100

# 특정 패턴 검색
xz -dc backup.sql.xz | grep "users"
```

### Q: Windows에서 xz를 사용할 수 없는데요?

A: 3가지 방법이 있습니다:
1. **7-Zip** 설치 (권장)
2. **Git Bash** 사용 (git 설치 시 포함, xz 포함 여부 확인)
3. **PowerShell 7+** 사용 (7z x 또는 xz 설치 시)

### Q: 압축을 나중에 활성화할 수 있나요?

A: 네, 언제든지 가능합니다:
- 기존 백업: 압축되지 않은 `.sql` 파일로 유지
- 새 백업: 압축된 `.sql.xz` 파일로 생성
- 두 형식 모두 복구 가능

---

## 모범 사례

### ✅ 권장사항

1. **프로덕션에서는 항상 압축 활성화**
   ```bash
   BACKUP_COMPRESS=true
   ```

2. **정기적인 용량 모니터링**
   ```bash
   # 매주 월요일 백업 크기 확인
   crontab: 0 9 * * 1 du -sh /backups/database | mail -s "백업 크기" admin@example.com
   ```

3. **압축 파일 직접 복구**
   ```bash
   # 임시 파일 생성 없이 바로 복구
   xz -dc backup.sql.xz | psql -d database
   ```

4. **복구 테스트**
   ```bash
   # 월 1회 복구 테스트
   createdb test_restore
   xz -dc latest_backup.sql.xz | psql -d test_restore
   dropdb test_restore
   ```

### ❌ 피해야 할 사항

1. **프로덕션에서 압축 비활성화**
   - 디스크 공간 부족 위험
   - 높은 스토리지 비용

2. **압축 파일을 수동으로 편집**
   - 파일 손상 위험
   - 압축 해제 → 편집 → 재압축 권장

3. **압축 해제 후 원본 삭제**
   ```bash
   # 잘못된 방법
   xz -d backup.sql.xz  # 원본 삭제됨
   
   # 올바른 방법
   xz -dk backup.sql.xz  # 원본 유지
   ```

---

## 관련 문서

- [데이터베이스 백업 가이드](./database-backup-guide.md)
- [SQL 복구 가이드](./sql-restore-guide.md)
- [TypeORM 백업의 장점](./typeorm-backup-benefits.md)

---

**마지막 업데이트**: 2026-01-21  
**버전**: 2.0.0 (xz 압축 지원)
