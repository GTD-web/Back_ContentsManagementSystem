...
다음 작업 수행 시 모든 콘텐츠의 번역 동기화가 자동으로 트리거됩니다:

- **언어 추가** (`POST /api/admin/languages`) - 새 언어가 활성 상태(`isActive: true`)로 추가된 경우
- **언어 활성화** (`PATCH /api/admin/languages/:id/active`) - 기존 언어를 활성화(`isActive: true`)한 경우

이를 통해 새로 추가되거나 활성화된 언어로 기존 콘텐츠의 번역이 자동으로 생성됩니다.
